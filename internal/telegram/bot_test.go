...
func TestBotRegisterCommands(t *testing.T) {
	b := setupTestBot(t)
	expected := []string{"/start", "/help", "/cancel", "/setup", "/day", "/week", "/calls", "/about", "/group", "/teacher", "/settings", "/image", "/buttons", "/formatter", "/forceparse", "/resetcache", "/eula", "/api", "/diff", "/flushCache", "/debug", "/send", "/trigger"}
