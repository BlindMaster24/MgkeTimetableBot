...
- `Bot.Run` picks the update transport: `internal/telegram/webhook.go` serves the `telegram.webhook` configuration (secret-token check, optional self-signed TLS, and `setWebhook` instead of `getUpdates`) and `runPolling` deletes a stale webhook first, so switching modes never hits the Telegram `409 Conflict`.
- telego owns no HTTP server on purpose: `telego.WebhookHandler` hands the raw JSON to the caller and documents that the caller validates the request and wraps the context in `context.WithoutCancel`. The webhook transport therefore keeps its own `http.Server` (read-header/read/write/idle timeouts, a 1 MB body cap, the constant-time secret check and the 405/401/413/500 status table) rather than a custom HTTP stack. The update channel is the backpressure knob: `telegram.webhook.buffer` sizes it and a full channel parks the POST until Telegram gives up and retries.
- shutdown flows from the single root context in `cmd/bot/main.go`: every stop signal cancels it (`cmd/bot/signals_unix.go` — `SIGINT`, `SIGTERM`, `SIGHUP`, `SIGQUIT`; `cmd/bot/signals_windows.go` — `os.Interrupt`, `SIGTERM`), and `runWebhook` drains in-flight webhook requests through `http.Server.Shutdown` (5 s window) before it returns. `shutdown complete` therefore means the listener is closed, which the `container` CI job relies on. `internal/telegram/fakeapi_test.go` covers both transports over real HTTP against the fake Bot API, including the drain and the released port.
