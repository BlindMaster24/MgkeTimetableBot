...
- `path` проверяется парсером самого `http.ServeMux`: путь, который сервер не сможет обслужить (например, с пробелом), бот отбивает при старте с понятной ошибкой, а не падает с паникой;
