...
`/incidents` показывает историю срабатываний парсера, календаря и API — что ломалось, когда, сколько длилось, чем закончилось и что помогло. История живёт в базе чатов (таблица `bot_state`, ключ `health.incidents`), потому что вопрос «когда это началось» задают уже после перезапуска бота, и в памяти он был бы потерян; хранятся последние 100 записей. Запись с иконкой `⚠️` — ещё открытая, `🔧` — закрытая ручным действием админа (например, нажатием кнопки переразбора), `✅` — восстановившаяся сама. Если у открытого сбоя есть кнопка-починка, команда показывает её под сообщением.
