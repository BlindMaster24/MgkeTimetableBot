...
import {
    CallsOverrideSource,
    getCallsManualReason,
    getCallsOverrideSource,
    setCallsManualReason,
    setCallsOverrideSource
} from './callsStore';
...
export type CallsUpdateEvent = {
    schedule: CallsSchedule;
    weekdaysChanged: boolean;
    saturdayChanged: boolean;
...
export class ParserService implements AppService {
    ...
    public lastSuccessUpdate(): number {
        const times = [raspCache.groups.update, raspCache.teachers.update];
    ...
    private async runActions(): Promise<boolean[]> {
        ...
        if (config.parser.calls?.enabled) {
            const due =
                this._forceCallsParse ||
                !raspCache.calls.update ||
                Date.now() - raspCache.calls.update >= config.parser.update_interval.calls * 1e3;
            if (due) {
                PARSER_ACTIONS.push(
                    this.parseCallsSchedule.bind(this, encodeURI(config.parser.endpoints.bellSchedule), raspCache.calls)
                );
            }
        }
        ...
        if (
            this._forceParse ||
            this._clearKeys ||
            config.parser.localMode ||
            !raspCache.team.update ||
            ...
            for (const i in config.parser.endpoints.team) {
                ...
                const action = this.parseTeam.bind(this, Number(i), encodeURI(url), raspCache.team);
    ...
    private async parseCallsSchedule(
        url: string,
        cache: CallsCache,
        report?: { siteParsed?: boolean; error?: Error; schedule?: CallsSchedule; updatedAtRaw?: string }
    ): Promise<boolean> {
        ...
        if (config.parser.localMode) {
            ...
            try {
                ...
                const error = this.attachParserContext(e, {
                    stage: 'fetch',
                    type: 'calls',
                    url,
                    parser: 'CallsParser'
        ...
        if (schedule && schedule.weekdays.length > 0) {
            ...
            if (preferSite && !config.parser.localMode) {
                ...
                if (now - lastNotified > 6 * 60 * 60 * 1e3) {
                    ...
                    const error = this.attachParserContext(new Error('calls schedule empty'), {
                        stage: 'parse',
                        type: 'calls',
                        url,
                        parser: 'CallsParser'
    ...
    private selectActiveCalls(updatedAtRaw?: string, options?: { skipNotify?: boolean }) {
        ...
        if (
            ...
            const weekdaysChanged =
                JSON.stringify(cache.active.schedule.weekdays) !== JSON.stringify(activeSchedule.weekdays);
            const saturdayChanged =
                JSON.stringify(cache.active.schedule.saturday) !== JSON.stringify(activeSchedule.saturday);
            cache.active = {
    ...
    private async parseTimetable(
        Parser: TimetableParser,
        url: string,
        cache: RaspEntryCache<Teachers | Groups>,
        fallbackParser?: TimetableParser
    ) {
        ...
        if (config.parser.localMode) {
            ...
            const content =
                doc.querySelector('#main-p .content') ||
            ...
            try {
                ...
                parseError = this.attachParserContext(
                    e,
                    Object.assign(
                        {
                            stage: 'parse',
            ...
            const v1Parser: TimetableParser =
                Parser === StudentParser || Parser === StudentParserV2 ? StudentParser : TeacherParser;
            ...
            if (!parsed) {
                if (parseError) {
                    ...
                    throw this.attachParserContext(
                        parseError,
                        Object.assign(
                            {
                                stage: 'parse',
            ...
            if (isV2Parser && config.parser.v2?.strict) {
                ...
                const validation = onParser(
                    usedParser,
                    validateGroups(parsed as Groups, maxLessons, sampleSize),
                    validateTeachers(parsed as Teachers, maxLessons, sampleSize)
                );
                if (!validation.ok) {
                    if (fallbackParser && usedParser !== fallbackParser) {
                        const fallbackParsed = parseWith(fallbackParser);
                        const fallbackValidation = onParser(
                            fallbackParser,
                            validateGroups(fallbackParsed as Groups, maxLessons, sampleSize),
                            validateTeachers(fallbackParsed as Teachers, maxLessons, sampleSize)
                        );
                        if (!fallbackValidation.ok) {
                            await this.writeSnapshot(url, 'bad');
                            throw this.attachParserContext(
                                new Error(`v2 validation failed: ${validation.errors.join('; ')}`),
                                Object.assign(
                                    {
                                        stage: 'validate',
                                        parserUsed: usedParser.name,
            ...
            if (shouldDiff && usedParser === Parser) {
                ...
                if (v1Parsed) {
                    ...
                    const diffLines = onParser(
                        Parser,
                        diffGroups(parsed as Groups, v1Parsed as Groups, limit),
                        diffTeachers(parsed as Teachers, v1Parsed as Teachers, limit)
                    );
                    if (diffLines.length) {
        ...
        if (config.parser.v2?.enabled && config.parser.v2?.quarantine?.enabled) {
            ...
            const totalLessons = entries.reduce((total: number, entry) => {
                return (
                    total +
                    entry.days.reduce((sum: number, day) => {
        ...
        const siteMinimalDayIndex: number = Math.min(
            ...Object.entries(data).reduce<number[]>((bounds: number[], [, entry]): number[] => {
        ...
        if (config.parser.v2?.enabled && weekPolicy !== 'preferCurrent') {
            ...
            for (const entry of entries) {
                const indexes = Array.from(
                    new Set(entry.days.map((day: GroupDay | TeacherDay) => WeekIndex.fromStringDate(day.day).valueOf()))
                );
                let targetIndex = currentWeekIndex;

                if (!indexes.includes(currentWeekIndex)) {
                    if (weekPolicy === 'closest') {
                        targetIndex = indexes.sort(
                            (a: number, b: number) => Math.abs(a - currentWeekIndex) - Math.abs(b - currentWeekIndex)
                        )[0];
                    }
                }
        ...
        const previousWeekIsFuture =
            previousWeekIndex > 0 && WeekIndex.fromWeekIndexNumber(previousWeekIndex).isFutureWeek();
        const shouldCheckWithdraw =
            previousWeekIsFuture && (!config.parser.v2?.enabled || weekPolicy === 'preferCurrent');
        ...
        for (const index in data) {
            ...
            if (!currentEntry) {
                ...
                for (const day of changed) {
                    ...
                    if (dayIndex.isToday()) {
                        ...
                        eventName = onParser<keyof ParserEvents>(Parser, 'updateGroupDay', 'updateTeacherDay');
                        ...
                        if (currentEntry.lastNoticedDay === dayIndex.valueOf()) {
                            ...
                            eventName = onParser<keyof ParserEvents>(Parser, 'addGroupDay', 'addTeacherDay');
                        }
                    }

                    if (eventName) {
                        const eventData = onParser<GroupDayEvent | TeacherDayEvent>(
                            Parser,
                            {
                                day: day as GroupDay,
                                group: index
            ...
            if (toArchive.length > 0) {
                flushLessons.push(
                    ...toArchive.map((day): ArchiveAppendDay => {
                        return onParser<ArchiveAppendDay>(
                            Parser,
                            {
                                type: 'group',
                                value: index,
        ...
        for (const index in cache.timetable) {
            ...
            entry.days = (entry.days as any).filter((day: GroupDay | TeacherDay): boolean => {
                ...
                const keep: boolean =
                    (preserveCurrentWeek && inCurrentWeek) ||
                    dayIndex.isNotPast() ||
                    dayIndex.valueOf() >= siteMinimalDayIndex;

                if (!keep) {
                    flushLessons.push(
                        onParser<ArchiveAppendDay>(
                            Parser,
                            {
                                type: 'group',
                                value: index,
        ...
        const maxWeekIndex = Math.max(
            ...(Object.values(cache.timetable) as (Group | Teacher)[]).map((entry) => {
    ...
    private async parseTeam(pageIndex: number, url: string, cache: TeamCache): Promise<boolean> {
        ...
        cache.names = Object.keys(cache.names)
            .sort()
            .reduce<Team>((obj, key) => {
