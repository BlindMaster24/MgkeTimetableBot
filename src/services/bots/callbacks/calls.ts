...
export default class CallsCallback extends AbstractCallback {
    ...
    async handler({ context, chat, keyboard }: CbHandlerParams | CmdHandlerParams) {
        let [showFull] = z.tuple([z.coerce.boolean()]).parse(context.payload ?? [false]);
        ...
        const maxLessons: number = Math.max(activeSchedule.saturday.length, activeSchedule.weekdays.length);

        let current: number | undefined;

        if ((chat.mode === 'parent' || chat.mode === 'student') && chat.group) {
            current = Math.max(...(raspCache.groups.timetable[chat.group]?.days.map((_) => _.lessons.length) || []));
        } else if (chat.mode === 'teacher' && chat.teacher) {
            current = Math.max(
                ...(raspCache.teachers.timetable[chat.teacher]?.days.map((_) => _.lessons.length) || [])
            );
        }
        ...
        return context.editOrSend(message.join('\n'), {
            keyboard: showFull
                ? undefined
                : keyboard.getKeyboardBuilder('CallsFull', true).add({
                      type: ButtonType.Callback,
                      text: '\u041F\u043E\u043A\u0430\u0437\u0430\u0442\u044C \u043F\u043E\u043B\u043D\u043E\u0441\u0442\u044C\u044E',
                      payload: this.payloadAction + JSON.stringify([Number(true)])
                  })
        });
    }
