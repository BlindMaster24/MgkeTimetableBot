...
export default class extends AbstractCommand {
    ...
    async handler({ context }: CmdHandlerParams) {
        ...
        if (!allowed)
            return context.send('В примере имеются лишние символы.\n' + `Список разрешённых: ${allowStrings}`);
        ...
        if (!correct) return context.send('Пример записан неправильно');
        ...
        if (error) return context.send('Произошла ошибка во время выполнения:\n' + `${error}`);

        if (result === Infinity) return context.send('бесконечность');

        if (typeof result !== 'number') return context.send('Выходящий тип данных почему-то не равен number');
