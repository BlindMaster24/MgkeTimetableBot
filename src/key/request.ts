...
export const appType = ['VK Bot', 'VK App', 'Telegram Bot', 'Viber Bot', 'Android App'];
...
export class RequestKey extends AbstractKey {
    ...
    public getKey(input: InputRequestKey) {
        ...
        buffer_writer.writeBuffer(
            this.packBits(this.v.toString(2).padStart(4, '0') + input.from.toString(2).padStart(4, '0'))
        );
        ...
        switch (input.from) {
            case FromType.VKBot: {
                ...
                buffer_writer.writeBuffer(
                    this.packBits(
                        String(+isIdEqu) + '0000000' // reserved
                    )
                );
